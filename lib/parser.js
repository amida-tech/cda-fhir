...
var dateFix = function (date) {
    if (date) {
        ...
        case 19: //20090227130000+0500
            return date.substr(0, 4) + '-' + date.substr(4, 2) + '-' + date.substr(6, 2) + 'T' + date.substr(8, 2) + ':' + date.substr(10, 2) + ':' + date.substr(12, 2) + 'Z' + date.substr(14, 3);
        }
    }
    return date;
};
...
var AssignedEntity = function (resource, templateId) {
    ...
    if (templateId && _.contains(templateId, '2.16.840.1.113883.10.20.22.4.87')) {
        ...
        this._self = {
            id: function (node) {
                ensureProperty.call(practitioner, 'identifier', true).push(node.attributes.root);
            },
            code: function (node) {
...
var SomeWithName = function (some) {
    ...
    this.name = function (node) {
        ...
        var name = {
            'use': recodeNameUse(node.attributes.use)
        };
