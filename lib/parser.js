...
var genericLocationHandler = function(location) {
  return {
      code: function(node) {
          location.type = {'coding':[makeCode(node)]};
      },
      addr: function(node) {
          location.address = {};
...
var ParticipantRole = function (resource, contextId) {
    ...
    this.templateId = function (node) {
        ...
        switch(node.attributes.root) {
            ...
            if(procedure) {
             var patient = findPatient(proto.bundle);
                var location = getResource('Location', {'id': 'Location/' + (serial++).toString()});
                            proto.bundle.entry.push({
                'resource': location
            });
            proto.composition.section.push({
...
var genericObservationHandler = function (observation, templateId) {
    return {
        ...
        targetSiteCode: function(node) {
            observation.bodySiteCodeableConcept = {'coding':[makeCode(node)]}; 
        },
        method:  function (node) {
            if(node.attributes.code){
                observation.method = {'coding':[makeCode(node)]};
            }
        },
        performer: function (node) {
             var patient = findPatient(proto.bundle);
              var practitioner = {
                ...
                ensureProperty.call(observation, 'performer',true).push({'reference':practitioner.id});
...
var Observation = function (typeCode, resource, param1, bundle, composition) {
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.22.4.13': // Procedure activity observation
            var procedure = findLastResourceOfType('Procedure');
            observation = resource;
            if(procedure) {
                ensureProperty.call(procedure, 'relatedItem').push({ 'reference':observation.id});
            }
            this._self = genericObservationHandler(observation);
            this._self.prototype = proto;
