...
var AssignedEntity = function (resource, templateId) {
    ...
    if (templateId && _.contains(templateId, '2.16.840.1.113883.10.20.22.4.87')) {
        ...
        this._self = {
            id: function (node) {
                if (node.attributes.root) {
                    ensureProperty.call(contact, 'identifier', true).push({
                        'value': node.attributes.root
                    });
                }
            },
        ...
        case '2.16.840.1.113883.10.20.22.4.47': // Family history death observation
            ...
            this._self = {
                value: function (node) {
                    ...
                    if (last) {
                        if (node.attributes['xsi:type'] === 'CD') {
                            last.outcome = _.merge(makeCode(node), last.outcome);
        ...
        case '2.16.840.1.113883.10.20.22.4.31': // Age observation (familyhistory)
            if (isInContextOf('2.16.840.1.113883.10.20.22.4.4')) { //Problem observation
                ...
                if (condition) {
                    this._self = {
                        code: function (node) {
                            condition.resource.onsetAge = _.merge(makeQuantity(node), condition.resource.onsetAge);
                        },
                        value: function (node) {
                            condition.resource.onsetAge = _.merge(valueFix(node), condition.resource.onsetAge);
                        }
                    };
                    this._self.prototype = proto;
                }
                ...
                this._self = {
                    value: function (node) {
                        last = _.last(ensureProperty.call(familyMemberHistory, 'condition', true));
                        if (last) {
                            last.onsetAge = _.merge(valueFix(node), last.onsetAge);
                        }
                    },
                    code: function (node) {
                        last = _.last(ensureProperty.call(familyMemberHistory, 'condition', true));
                        if (last) {
                            last.onsetAge = _.merge(makeQuantity(node), last.onsetAge);
                        }
                    }
                };
                this._self.prototype = proto;
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.22.4.17':
            ...
            if (!resource.prescription) {
                ...
                medicationPrescription = getResource('MedicationPrescription', {
                    'id': 'MedicationPrescription/' + (serial++).toString()
                });
                store2bundle(medicationPrescription, patient.id);
            ...
            this._self = {
                ...
                repeatNumber: function (node) {
                    ensureProperty.call(medicationPrescription, 'dispense').numberOfRepeatsAllowed = Number(node.attributes.value);
                },
...
var SubstanceAdministration = function () {
    ...
    this.templateId = function (node) {
        ...
        case '2.16.840.1.113883.10.20.22.4.52': //Immunization activity
            ...
            store2bundle(immunization, _patient.id);
        ...
        case '2.16.840.1.113883.10.20.22.4.16': //Medication activity
            ...
            var _medicationPrescription = getResource('MedicationPrescription', {
                'id': 'MedicationPrescription/' + (serial++).toString(),
                'patient': {
                    'reference': _patient.id
                },
            ...
            store2bundle(_medicationPrescription, _patient.id);
            ...
            store2bundle(_medicationAdministration, _patient.id);
...
var ClinicalDocument = function (patientId) {
...
var Start = function (patientId) {
    var clinicalDocument = new ClinicalDocument(patientId);
